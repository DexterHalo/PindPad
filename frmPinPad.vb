...
Public Class frmPinPad
    ...
    Private Sub btnProcesar_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnProcesar.Click
        ...
        Else
            ...
            Else
                ckNet.MsgInf("Se requiere la inicializacion de la pinpad")
            End If

        End If
    End Sub
    Private Function CancelarVenta(idOperation As String, referencia As String) As Boolean
        Try
            Dim url As String = My.Settings.ApiUrl + "/api/Cancelation"
            ...
            Else
                ...
                ckNet.MsgInf("❌ Error en cancelación: " & cancelResponse.detailError)
                Return False
            End If

        Catch ex As Exception
            ckNet.MsgInf("❌ Excepción al cancelar venta: " & ex.Message)
            Return False
        End Try
    End Function


    Private Function EnviarVenta(monto As String, referencia As String) As Boolean
        Try
            Dim url As String = My.Settings.ApiUrl + "/api/Sale"

            Using client As New HttpClient()
                ' Validar token
                If String.IsNullOrWhiteSpace(Token) Then
                    ckNet.MsgInf("⚠️ Token no encontrado. Primero ejecuta la configuración.")
                    Return False
                End If
                ...
                Dim jsonBody As String = JsonConvert.SerializeObject(requestData)
                ...
                ' Ejecutar POST sincrónico
                Dim response As HttpResponseMessage = client.PostAsync(url, content).Result

                If response.IsSuccessStatusCode Then
                    Dim responseBody As String = response.Content.ReadAsStringAsync().Result
                    Dim resultado = JsonConvert.DeserializeObject(Of SaleResponse)(responseBody)
                ...
                Else
                    ...
                    ckNet.MsgInf($"❌ Error HTTP {response.StatusCode}: {errorBody}")
                    Return False
                End If
            End Using
